...
class BabyTimeAccessibilityService : AccessibilityService() {
    ...
    private fun tryExecuteNext() {
        ...
        if (!success) {
            ...
            if (step is ActionStep.DismissKeyboard && PendingActionQueue.hasNext()) {
                handler.postDelayed({ tryExecuteNext() }, 300)
            }
        }
    }
    ...
    private fun dismissKeyboardIfVisible(): Boolean {
        val imeVisible = windows?.any { it.type == AccessibilityWindowInfo.TYPE_INPUT_METHOD } ?: false
        if (imeVisible) {
    ...
    override fun onInterrupt() {
        Log.w("BabyVoice", "service interrupted")
    }
}
